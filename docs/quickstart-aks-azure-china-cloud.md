# Quickstart: Open Service Broker for Azure on an Azure Kubernetes Service cluster in Azure China Cloud

This quickstart walks through using the Open Service Broker for Azure (OSBA) to
deploy WordPress on an [Azure Kubernetes Service (AKS) in Azure China Cloud](https://azure.microsoft.com/en-us/services/container-service/) managed cluster.
...
* A [Microsoft Azure account] in Azure China Cloud(https://azure.microsoft.com/en-us/global-infrastructure/china/).
...
1. Before we can use Helm to install applications such as Service Catalog and
    ...
    helm init -i gcr.azk8s.cn/kubernetes-helm/tiller:v2.9.1 --service-account tiller
    ```
...
have it! Learn how to [contribute a new chart](https://github.com/Azure/helm-charts#creating-a-new-chart) to our helm repository.
